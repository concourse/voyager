...
type SchemaAdapter interface {
	// ConvertFromOldSchema converts from an old database schema to
	...
	//
	// The version returned by ConvertFromOldSchema will be used as the
	// first version in the migrations_history table.
	//
	// It is called every time `voyager.Migrate` is called, before any
	// migrations are run, so implementations are responsible for
	// returning a sane starting version in the case that the old schema
	// is no longer used.
	ConvertFromOldSchema(*sql.DB, int) (int, error)

	// ConvertToOldSchema converts from the new migrations_history schema
	// to the old schema. It exists to support migrating back down to a
	// previous migration version.
	//
	// It is called every time `voyager.Migrate` is called, after all
	// migrations are run.
	ConvertToOldSchema(*sql.DB, int) error
...
func (m *migrator) Migrate(logger lager.Logger, db *sql.DB, toVersion int) error {
	...
	if m.adapter != nil {
		existingDBVersion, err = m.adapter.ConvertFromOldSchema(db, toVersion)
		if err != nil {
			return err
		}
	}
	...
	if currentVersion <= toVersion {
		...
		if m.adapter != nil {
			err = m.adapter.ConvertToOldSchema(db, toVersion)
			if err != nil {
				return err
			}
		}
